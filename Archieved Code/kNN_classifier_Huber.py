...
class TwoStageClassifierRegressor:
    def __init__(self):
        ...
        self.best_ext_classifier = None
        self.best_core_rf_regressor = None
        self.best_ext_svr_model = None
    ...
    def read_train_csv(self, file_path: str) -> pd.DataFrame:
        ...
        self.is_extreme_flag = np.abs(self.y_train) > 200
    ...
    def extreme_points_classifier(self):
        ...
        knn_hyperparams = {
            'knn__n_neighbors': [1],  # Number of neighbors
            'knn__weights': ['uniform', 'distance'],  # How to weight neighbors
            'knn__metric': ['euclidean', 'manhattan']  # Distance metric
        }
        knn_grid = GridSearchCV(ext_clf_pipeline, knn_hyperparams,
                               scoring="accuracy",
                               cv=kf,
                               n_jobs=-1,verbose=1)
        knn_grid.fit(self.x_train, self.is_extreme_flag)
        print(f"RF Classifier best params: {knn_grid.best_params_}")
        print(f"RF Classifier best score: {knn_grid.best_score_}")
        self.best_ext_classifier = knn_grid.best_estimator_
        print(f"RF Classifier best model: {self.best_ext_classifier}")

    def ext_svr_model(self):
        ...
        svr_grid.fit(self.x_ext_train, self.y_ext_train)
        ...
        self.best_ext_svr_model = svr_grid.best_estimator_
        print(f"Core SVR best model: {self.best_ext_svr_model}")

    def core_rf_model(self):
        ...
        rf_grid = GridSearchCV(ext_rf_pipeline, rf_hyperparams,
                               ...
                               refit='neg_mean_squared_error',
                               cv=kf, n_jobs=-1,verbose=1)
        rf_grid.fit(self.x_core_train, self.y_core_train)
        ...
        self.best_core_rf_regressor = rf_grid.best_estimator_
        print(f"RF Regressor best model: {self.best_core_rf_regressor}")

    def classifier_prediction(self):
        if self.best_ext_classifier is None:
            raise ValueError("best_ext_rf_classifier is empty. Train the extreme points classifier first.")
        orig_pred = self.best_ext_classifier.predict(self.x_train)
        print(f"number of train test extreme points:{orig_pred.sum()}")
        ext_pred = self.best_ext_classifier.predict(self.test_data)
        print(f"number of test set extreme points: {ext_pred.sum()}")

    def two_stage_prediction(self):
        if self.best_ext_classifier is None:
            raise ValueError("best_ext_rf_classifier is empty. Train the extreme points classifier first.")
        if self.best_core_rf_regressor is None:
        ...
        if np.any(ext_pred):
            preds[ext_pred] = self.best_ext_svr_model.predict(self.test_data[ext_pred])
        if np.any(~ext_pred):
            preds[~ext_pred] = self.best_core_rf_regressor.predict(self.test_data[~ext_pred])
...
def main():
    ...
    #ts.save_predictions("knn_svr_rf_submission.csv")

    plt.figure(figsize=(10, 6))
    plt.scatter(ts.test_data['x9'], ts.final_preds)
